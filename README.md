...
```
저는 SI회사에서 6개월정도 하이브리드 앱에 들어가는 웹뷰와 관리자 페이지를 개발하였고, 졸업 후에는(3년 2개월 정도) ASP.NET(SaaS형 프로그램)과 PHP(결제 시스템), React + node(SPA)로 개발 및 운영 경험을 가지고 있습니다. SPA프로젝트를 하면서 React의 매력을 느꼈고,  프론트앤드분야의 다양한 경험을 쌓고자 지원하게 되었습니다. 

React는 도너스에서 재직중에 작년 8월 정도에 관련 프로젝트를 2개정도 진행하였습니다.
**첫번째**는 Box 라는 프로젝트 였고, 처음 진행하는 React 프로젝트였고 react, redux 라이브러리를 이용해서 구현한(개발,배포,운영) 프로젝트 였습니다.

**두번째**는 Pay(PHP)라는 레거시 프로젝트를 React로 변환하는 작업을 진행하였습니다. 프로토 타입 개발으로 개발의 가능성을 확인하는 개발작업이었고, PasS에 배포까지 진행하였습니다.
